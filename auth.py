...
def init_db():
    ...
    if count == 0:
        # Mot de passe du superadmin initial : variable d'environnement de préférence.
        # À défaut (démo locale), génération d'un mot de passe aléatoire affiché une
        # seule fois — jamais de secret en dur dans le code source.
        admin_email = os.environ.get("ADMIN_EMAIL", "admin@sonatrach.dz")
        ...
        conn.execute(
            ...
            (admin_email, "ADMIN", "Super", "superadmin",
             hash_password(admin_pwd), datetime.now().isoformat())
        )
        conn.commit()
        if generated:
            print(f"✓ Superadmin créé : {admin_email}")
            print(f"  ⚠ Mot de passe généré (à noter maintenant) : {admin_pwd}")
        else:
            print(f"✓ Superadmin créé : {admin_email} (mot de passe via ADMIN_PASSWORD)")
    else:
