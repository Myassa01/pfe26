...
class RAGPipeline:
    ...
    def query(
        ...
        # ── Garde-fou de confidentialité (niveau 1 : la question) ────────────
        # Bloque les demandes de données sensibles (matricule, salaire…) pour
        # les utilisateurs non privilégiés, avant tout traitement.
        from .security import is_sensitive_question

        refusal = is_sensitive_question(question, role)
        if refusal:
            logger.info("Sécurité: question sensible refusée (role=%s): %r", role, question[:60])
            return {
                "question":          question,
