...
def build_nodes(components: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def retrieve_node(state: GraphState) -> dict:
        ...
        # ── Expansion de requête pour les questions composées ─────────────
        # Pour les questions avec "et que/qu", on fait une 2ème recherche sur
        # la première partie seule et on fusionne. Cela évite que la 2ème
        # partie dilue le signal de retrieval (corrige M016 non-déterministe).
        import re as _re
        _split = _re.split(r"\s+et\s+(?:que?|qu['e])\s+", search_query, maxsplit=1, flags=_re.IGNORECASE)
        if len(_split) == 2 and len(_split[0].strip()) >= 10:
            sub_query  = _split[0].strip()
            sub_emb    = embedder.embed_single(sub_query)
            sub_dense  = vector_store.search(sub_emb, k=k_dense)
            sub_sparse = bm25.search(sub_query, k=k_sparse)
            sub_hybrid = reciprocal_rank_fusion(sub_dense, sub_sparse, k=config.rrf_k)
            seen_ids   = {d["id"] for d in hybrid}
            extra      = [d for d in sub_hybrid if d["id"] not in seen_ids]
            hybrid     = hybrid + extra
            logger.info("  [retrieve] Expansion requête → +%d chunks", len(extra))
